'use client';
import { createComic } from '@/auth/comic.service';
...
const CreateComicPage: React.FC = () => {
    ...
    const handleUpload = async () => {
        if (!title.trim()) {
            setMessage({
                type: 'error',
                text: 'Vui lòng nhập tiêu đề truyện!'
            });
            return;
        }
        ...
        try {
            const response = await createComic(title.trim(), selectedFile);
            console.log('Tạo comic thành công:', response);
            setMessage({
                type: 'success',
                text: 'Comic đã được tạo thành công!'
            });
            setTitle('');
    ...
    return (
        <div className='max-w-md mx-auto mt-10 p-6 bg-white rounded-xl shadow-md border border-gray-200'>
            ...
            <div className='flex flex-col items-center space-y-4'>
                <div className='w-full'>
                    ...
                    <input
                        ...
                        placeholder='Nhập tiêu đề comic'
            ...
            ${loading ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 shadow-lg active:scale-95'}`}
                >
                    {loading ? 'Đang tạo comic...' : 'Tạo comic'}
                </button>
            </div>
        </div>
    );
